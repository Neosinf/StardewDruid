...
namespace StardewDruid.Handle
{
    public class TrackHandle
    {
        ...
        public Character.Character TrackSubject()
        {
        ...
        public void TrackPlayer(int Offset)
        {
            ...
            Character.Character subject = TrackSubject();
            ...
            if (subject is Recruit villager)
            {
                ...
                if (villager.TrackConflict(followPlayer))
                {
        ...
        public bool WarpToPlayer(int direction = -1)
        {
            ...
            GameLocation chooseLocation = followPlayer.currentLocation;

            Vector2 choosePosition = followPlayer.Position;

            if (chooseLocation is FarmHouse || chooseLocation is IslandFarmHouse)
            {

                CharacterHandle.locations home = CharacterHandle.CharacterHome(subject.characterType);

                chooseLocation = Game1.getLocationFromName(CharacterHandle.CharacterLocation(home));

                choosePosition = CharacterHandle.CharacterStart(home, subject.characterType);
            ...
            if (direction == -1 && nodes.Count > 0)
            {
                ...
                direction = ModUtility.DirectionToTarget(choosePosition, nodes.Keys.Last())[2];
            ...
            if (direction == -1 && trackQuadrant == 0)
            {
                ...
                direction = (ModUtility.DirectionToCenter(chooseLocation, choosePosition)[2] + 4) % 8;
            ...
            Vector2 center = ModUtility.PositionToTile(choosePosition);

            // get occupiable tiles

            List<Vector2> options = ModUtility.GetOccupiableTilesNearby(chooseLocation, center, direction, 1, 2);

            // check who else might warp there

            List<Vector2> occupied = new();

            foreach (KeyValuePair<CharacterHandle.characters, Character.Character> friends in Mod.instance.characters)
            {
                ...
                if (friends.Value.currentLocation.Name != chooseLocation.Name) { continue; }

                occupied.Add(friends.Value.occupied);

            }

            foreach (KeyValuePair<CharacterHandle.characters, Character.Character> friends in Mod.instance.dopplegangers)
            {
            ...
            if (options.Count > 0)
            {
                ...
                foreach (Vector2 warppoint in options)
                {
                    ...
                    CharacterMover mover = new(subject, chooseLocation, warppoint * 64, true)
                    {

                        warp = subject.warpDisplay,
