...
// Type: StardewDruid.Dialogue.Jester
  ...
  public class Jester : StardewDruid.Dialogue.Dialogue
  {
    ...
    public override void DialogueApproach()
    {
      ...
      else
      {
        ...
        GameLocation.afterQuestionBehavior questionBehavior = new GameLocation.afterQuestionBehavior((object) jester, __vmethodptr(jester, AnswerApproach));
      ...
      switch (str.Length)
      {
        ...
        case 6:
          switch (str[0])
          {
            case 'a':
              ...
              DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(ReplyAccept)), 100);
              return;
            case 'q':
              if (!(str == "quests"))
                return;
            ...
            case 'r':
              if (!(str == "refuse"))
                return;
              // ISSUE: method pointer
              DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(ReplyRefuse)), 100);
              return;
            default:
              return;
          }
          ...
          switch (str[0])
          {
            case 'i':
              if (!(str == "introtwo"))
                return;
              // ISSUE: method pointer
              DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(DialogueIntroTwo)), 100);
              return;
            case 'r':
              if (!(str == "relocate"))
                return;
          ...
          switch (str[0])
          {
            case 'T':
              if (!(str == "Thanatoshi"))
                return;
              // ISSUE: method pointer
              DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(ReplyThanatoshi)), 100);
              return;
            case 'i':
              if (!(str == "introthree"))
                return;
              // ISSUE: method pointer
              DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(DialogueIntroThree)), 100);
              return;
            default:
              return;
          }
        case 11:
          if (!(str == "afterQuarry"))
            break;
          // ISSUE: method pointer
          DelayedAction.functionAfterDelay(new DelayedAction.delayedBehavior((object) this, __methodptr(ReplyAfterQuarry)), 100);
          break;
      }
    }
    ...
    {
      Game1.drawDialogue((NPC) this.npc, "Great! Now go across this bridge and descend into that dark dangerous dungeon over there. I'll meet you back on the farm, in that warmer, safer cave with the walking wood man in it. (You will need to have gained the Golden Scythe from the quarry tunnel before starting Jester's lessons)");
      this.CompleteIntro();
    }

    public void ReplyRefuse()
    {
      Game1.drawDialogue((NPC) this.npc, "Hehehe... I like you already! But you cannot escape this Fate, literally, and, well literally. When you've finished exploring the cave over there, come find me on your farm, in the cave with the walking wood man. (You will need to have gained the Golden Scythe from the quarry tunnel before starting Jester's lessons)");
      this.CompleteIntro();
    }
    ...
    {
      Game1.drawDialogue((NPC) this.npc, "The monsters that came out of that portal... they come from a realm adjacent to this one. (Jester smirks) I tried to go through myself, but something barred me from passing through. If I'm to pursue this mystery further, I'll have to figure out another way to the Ethereal plane. (Thank you for playing Stardew Druid: Rite of the Fates! Subscribe or join discord to get updates on the next installment - Neosinf)");
    }

    public void ReplyThanatoshi()
    {
      Game1.drawDialogue((NPC) this.npc, "Thanatoshi is one of my distant kin. He fought in this valley, a long time ago, but I've never had the chance to ask him about why or what happened... he vanished.(Jester stares through you) It seems a dusty statue in a dungeon is all that remains of the deadly Thanatoshi...");
    }
  }
}
